export const pt = {
  // Header
  header: {
    home: "Início",
    about: "Sobre",
    services: "Serviços",
    values: "Valores",
    contact: "Contato",
  },

  // Hero
  hero: {
    title: "Conectando o Brasil ao mundo",
    subtitle: "Soluções completas em comércio exterior",
    description: "Assessoria especializada em importação e exportação com atendimento em português, inglês e espanhol. Produtos premium para o mercado de alto-falantes.",
    servicesBtn: "Nossos serviços",
    contactBtn: "Fale conosco",
    highlight1Title: "Comércio global",
    highlight1Desc: "Expertise em importação e exportação",
    highlight2Title: "Experiência comprovada",
    highlight2Desc: "Desde 2009 no mercado",
    highlight3Title: "Produtos premium",
    highlight3Desc: "Qualidade internacional garantida",
  },

  // About
  about: {
    title: "Nossa história",
    subtitle: "Desde 2009 conectando o Brasil ao mundo",
    milestone1Title: "Fundação",
    milestone1Desc: "Início das atividades em comércio exterior",
    milestone2Title: "Expansão global",
    milestone2Desc: "Parcerias internacionais consolidadas",
    milestone3Title: "Especialização",
    milestone3Desc: "Foco em produtos para alto-falantes",
    historyTitle: "Quem somos",
    historyText: "A Global Key foi fundada em 2009 por Sérgio Dias, um profissional com vasta experiência no mercado internacional. Com mais de 30 anos de carreira, incluindo 18 anos na Philips do Brasil, onde atuou em diversas áreas como importação, exportação, desenvolvimento de fornecedores e engenharia de processos, Sérgio traz uma bagagem única para a empresa.\n\nA trajetória profissional do fundador inclui uma passagem significativa pela Philips na China, onde passou três anos desenvolvendo fornecedores e aprimorando processos de fabricação. Essa experiência internacional foi fundamental para estabelecer as bases sólidas da Global Key.\n\nHoje, a empresa se dedica a dois principais segmentos: assessoria completa em comércio exterior e comercialização de produtos especializados para reparos de alto-falantes, atendendo o mercado brasileiro com qualidade internacional.",
    objectivesTitle: "Objetivos futuros",
    objective1: "Expandir parcerias na China para ampliar o portfólio de produtos",
    objective2: "Desenvolver linha premium de componentes para alto-falantes",
    objective3: "Fortalecer presença no mercado nacional",
  },

  // Services
  services: {
    title: "Nossos serviços",
    description: "Soluções completas em comércio exterior e produtos especializados",
    tab1: "Assessoria internacional",
    tab2: "Produtos",
    
    // Consulting
    consultingTitle: "Assessoria completa em importação e exportação",
    consultingIntro: "Oferecemos suporte completo para sua operação de comércio exterior, desde o planejamento até a entrega final.",
    
    planningTitle: "Planejamento",
    planning1: "Análise de viabilidade econômica",
    planning2: "Estudo de mercado e fornecedores",
    planning3: "Classificação fiscal de produtos",
    planning4: "Cálculo de custos e tributos",
    
    operationalTitle: "Operacional",
    operational1: "Negociação com fornecedores internacionais",
    operational2: "Gestão de documentação e licenças",
    operational3: "Acompanhamento de embarques",
    operational4: "Desembaraço aduaneiro",
    operational5: "Logística integrada",
    
    languagesTitle: "Idiomas",
    languagesText: "Atendimento especializado em português, inglês e espanhol",
    
    // Products
    productsTitle: "Produtos premium para reparos de alto-falantes",
    productsIntro: "Linha completa de materiais de alta qualidade para o mercado profissional.",
    
    product1Name: "Polyimide Film (Kapton)",
    product1Specs: "Espessuras: 25µm, 50µm, 75µm e 125µm",
    product1Desc: "Filme de poliimida de alta temperatura, ideal para bobinas de voz e isolamento térmico em alto-falantes profissionais.",
    
    product2Name: "Phenolic Fiber",
    product2Specs: "Disponível em diversas espessuras",
    product2Desc: "Fibra fenólica de alta resistência mecânica e térmica, perfeita para estruturas de cone e aplicações que exigem estabilidade dimensional.",
    
    product3Name: "CCAW Wire (Copper Clad Aluminum Wire)",
    product3Specs: "Diversos diâmetros disponíveis",
    product3Desc: "Fio de alumínio revestido com cobre, oferecendo excelente relação peso/performance para bobinas de voz de alto-falantes.",
    
    product4Name: "Finishing Papers",
    product4Specs: "Gramaturas variadas",
    product4Desc: "Papéis especiais para acabamento e reparo de cones, com tratamento específico para resistência e durabilidade.",
    
    comingSoonTitle: "Em breve: Cones e Reparos",
    comingSoonDesc: "Estamos expandindo nossa linha de produtos para incluir cones completos e kits de reparo profissionais.",
    
    disclaimer: "Todos os produtos são equivalentes técnicos. Marcas mencionadas são de propriedade de seus respectivos donos e são citadas apenas para referência de equivalência técnica.",
  },

  // Values
  values: {
    title: "Nossos valores",
    missionTitle: "Missão",
    missionText: "Somos uma organização global em busca de oferecer produtos e serviços excepcionais, de forma descomplicada, ágil e moderna, com a finalidade de criar vínculos fortes e duradouros com nossos clientes, fornecedores e colaboradores.",
    visionTitle: "Visão",
    visionText: "Sermos referência na comercialização de produtos nacionais ou importados, bem como na prestação de serviços no Brasil ou no exterior.",
    valuesTitle: "Valores",
    value1: "Respeitar o indivíduo, independentemente de suas características",
    value2: "Buscar superar as expectativas de nossos clientes, fornecedores e parceiros",
  },

  // Contact
  contact: {
    title: "Entre em contato",
    subtitle: "Estamos prontos para atender você",
    phone: "Telefone",
    email: "E-mail",
    address: "Endereço",
    addressText: "São Paulo, SP - Brasil",
    hours: "Horário",
    hoursText: "Seg - Sex: 9h às 18h",
    ctaTitle: "Fale conosco pelo WhatsApp",
    ctaDescription: "Atendimento rápido e personalizado",
    ctaButton: "Iniciar conversa",
    followTitle: "Siga-nos",
    followDescription: "Acompanhe nossas novidades",
    company: "Global Key Comercio e Serviços LTDA",
    cnpj: "CNPJ: 11.104.845/0001-90",
  },

  // Gallery
  gallery: {
    title: "Galeria de Produtos",
    description: "Conheça alguns de nossos produtos de alta qualidade para a indústria elétrica.",
  },

  // WhatsApp
  whatsapp: {
    message: "Olá! Gostaria de saber mais sobre os serviços da Global Key.",
    ariaLabel: "Falar no WhatsApp",
  },

  // Footer
  footer: {
    description: "Excelência em consultoria internacional e produtos especializados para reparos de alto-falantes.",
    quickLinks: "Links Rápidos",
    contact: "Contato",
    copyright: "© 2025 Global Key Comercio e Serviços LTDA. Todos os direitos reservados.",
  },
};
